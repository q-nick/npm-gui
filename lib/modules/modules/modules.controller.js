...
function whenGetVersions(req, res) {
    ...
    var output = '';
    var child = exec('npm ls --depth=0 --json',
        function(error, stdout, stderr) {
            output = JSON.parse(stdout).dependencies;
        });

    var outputB = '';
    child.on('close', function() {
        var childOutdated = exec('npm outdated --json',
            function(error, stdout, stderr) {
                outputB = JSON.parse(stdout);
                for (var key in outputB) {
                    if (outputB.hasOwnProperty(key)) {
                        if(outputB[key].wanted !== outputB[key].current){
                            output[key].wanted = outputB[key].wanted;
                        }
                        output[key].latest = outputB[key].latest;
                    }
                }
...
function whenGetNSP(req, res) {
    ...
    var output = {};
    var child = exec('nsp check --output json',
        function(error, stdout, stderr) {
            if(stderr){
                helpers.buildObjectFromArray(JSON.parse(stderr), output, 'module')
            }
        });

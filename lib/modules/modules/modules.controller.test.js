...
describe('Modules Controller', function() {
    before(function() {
        ...
        sinon.stub(CommandsMock, 'run', function(command) {
            return CommandsMock.runStubs[command](arguments);
        });
    ...
    describe("NSP- GET", function() {
        it('should return insecure dependency from nsp command', function() {
            //preparation
            CommandsMock.runStubs[commands.nsp.check] = sinon.stub().returnsPromise();
            CommandsMock.runStubs[commands.nsp.check].resolves({
    ...
    describe("Versions - GET", function() {
        it('should return full info about versions', function() {
            //preparation
            CommandsMock.runStubs[commands.npm.ls] = sinon.stub().returnsPromise();
            CommandsMock.runStubs[commands.npm.ls].resolves({
                stdout: '{' +
                    '   "name": "npm-gui",' +
                    '   "version": "x.x.x",' +
                    '   "dependencies": {' +
            ...
            CommandsMock.runStubs[commands.npm.outdated] = sinon.stub().returnsPromise();
            CommandsMock.runStubs[commands.npm.outdated].resolves({
                stdout: '{' +
                    '   "angular": {' +
                    '       "current": "2.4.0",' +
                    '       "wanted": "2.4.5",' +

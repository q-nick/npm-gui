...
module.exports.reinstallAllModules = reinstallAllModules;
...
function reinstallAllModulesForRepo(repo) {
    return Rx.Observable.create(function(observer) {
        ...
        var rm = Rx.Observable.fromCallback(rimraf);
        var folder = (repo === 'bower') ? 'bower_components' : 'node_modules';
        var removeFolderSource = rm(ProjectService.getPath() + '/' + folder);
...
function reinstallAllModules() {
    return Rx.Observable.create(function(observer) {
        ...
        var npmReinstallSource = reinstallAllModulesForRepo('npm');
        var bowerReinstallSource = reinstallAllModulesForRepo('bower');

        var bothSource = Rx.Observable.concat(npmReinstallSource, bowerReinstallSource);

...
describe('Global Packages', () => {
  it('all actions should run correctly', async () => {
    ...
    // full list
    const responseFull = await api(app.server).get(
      '/api/global/dependencies/full',
    );
    expect(responseFull.status).to.equal(HTTP_STATUS_OK);
    const npmGuiTestsFull = responseFull.body.find(
      (d: any) => d.name === 'npm-gui-tests',
    ); // eslint-disable-line
    ...
    // uninstalling
    const responseUninstall = await api(app.server).delete(
    ...
    const responseSimple2 = await api(app.server).get(
      '/api/global/dependencies/simple',
    );
    expect(responseSimple2.status).to.equal(HTTP_STATUS_OK);
    const npmGuiTestsSimple2 = responseSimple2.body.find(
      (d: any) => d.name === 'npm-gui-tests',
    ); // eslint-disable-line
    expect(npmGuiTestsSimple2).to.eq();
  });
});
